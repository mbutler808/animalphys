...
You will use the sphygnomanometer cuff to cut off circulation in the leg for 30 sec. at the upper thigh. The respiratory belt transducer senses stretch and can be used to measure __calf volume__ (@fig-calf) __before__ inflating the cuff, __during 30 sec of inflation__, and __after deflating__ the cuff.  
::: 
...
2.  Rapidly __Inflate__ the cuff to __60 mmHg__ (or whatever pressure feels tight enough to restrict blood flow for the subject [I used 80 mmHg], should be same for all measurements), 
3.  __Hold pressure__ for __exactly 20 seconds__   (_NOTE: You may have to gently squeeze the bulb to keep pressure at 80 mmHg._) 
...
5.  Quickly release all cuff pressure, and comment ["deflated"]{style="color:blue"}. Record for 30 sec or until leg volume stabilizes. 
...
4. __Simulated dive:__ 
    ...
    c. Rapidly release all cuff pressure. Comment ["deflated"]{style="color:blue"}, and record for 30 sec.  
5.  __Recovery:__ Signal to the subject to resurface and breathe normally with face just above water. Comment ["recovery"]{style="color:blue"} and record for 10 sec or until until leg volume stabilizes. Perform a leg volume measurement:
    a.  Rapidly inflate cuff to 60mmHg, comment ["cuff inflated"]{style="color:blue"}.  
    b.  Record for 20 sec. 
    c. Rapidly release all pressure. Comment ["deflated"]{style="color:blue"}, and record for 30 sec.  
6.  Stop recording and allow subject to dry face. 
...
3. __Breath hold:__ 
    ...
    c. Rapidly release all pressure. Comment ["deflated"]{style="color:blue"}, and record for 30 sec.  
4.  __Recovery:__ Signal to the subject to breathe normally with face just above water. Comment ["recovery"]{style="color:blue"} and record for 10 sec or until until leg volume stabilizes.  Perform a leg volume measurement. 
    a.  Rapidly inflate cuff to 60mmHg, comment ["cuff inflated"]{style="color:blue"}.  
    b.  Record for 20 sec. 
    c. Rapidly release all pressure. Comment ["deflated"]{style="color:blue"}, and record for 30.  
5.  Stop recording. 
...
1.  First analyze the __Dive Response__ data. 
...
2. Add a channel under the __Channel Settings__ menu and set it show the __heart rate__ in beats per minute (BPM) using the __cyclic measurement menu__.  
...
4. Collect __heart rate__ and __pulse amplitude__ for:
   a. rest
   b. 15 sec into the dive
   c. just before the end of the dive
   d. 30 sec after the end of the dive (recovery)
5. Conduct this analysis for the __Breath Hold__ experiment.
...
1.  Collect the leg volume difference for the __control__, during the __dive__, and __recovery__ from the dive ($\Delta LV_{control}$, $\Delta LV_{treatment}$, and $\Delta LV_{recovery}$) . Tabulate as in Table @tbl-divedata.
2.  Calculate the relative leg volumes for dive vs. control and recovery vs. control ($\Delta LV_{treatment}/ \Delta LV_{control}$, and $\Delta LV_{recovery}/ \Delta LV_{control}$ ). 
3.  For the __breath hold experiment__, use the __recovery from the dive__ value as the __control__ for breath hold. 
4.  Collect the leg volume data as before and tablulate as in @tbl-breathholddata
5.  Do the same for your additional experiment.

| Subject name   | $\Delta LV_{control}$ (mV)   | $\Delta LV_{dive}$ (mV) | $\Delta LV_{recovery}$ (mV)  | Dive/Control | Recovery/Control |
| ----  | ----  | ----  | ----  | ---- | ---- |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |

: Effects of diving on peripheral circulation {#tbl-divedata}


| Subject name   | $\Delta LV_{control}$ (mV)   | $\Delta LV_{breathhold}$ (mV) | $\Delta LV_{recovery}$ (mV)  | Breath Hold/Control | Recovery/Control |
| ----  | ----  | ----  | ----  | ---- | ---- |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |

: Effects of breath hold on peripheral circulation. Use the recovery LV value from the dive experiment for the control.  {#tbl-breathholddata}
