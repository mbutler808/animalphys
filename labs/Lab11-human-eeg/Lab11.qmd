...
3.  Attach the leads of the EEG Flat Electrodes to the Earth, CH1 NEG and POS pins closest to the labeled side on the Bio Amp Cable.  Channel 1 “positive” and “negative” electrodes will attach on either side of the forehead. Channel 2 electrodes will lead to either side of the inion (the bump on the back of the head above the neck) and the Earth will lead to the temple.  Refer to Figure 1 for proper placement, but do not attach them to the volunteer.  Follow the color scheme on the Bio Amp Cable.

4. Remove any jewelry from the volunteer’s face, ears, and neck.  Use a ballpoint pen to mark a small cross on the skin on the back of the head, forehead, and temple.  Use Figure 1 as a guide.  Abrade the skin with Abrasive Gel or Pad.  This is important as abrasion helps reduce the skin’s resistance.
...
3.  Tell the volunteer to open both eyes.  Immediately press Return/Enter to add the comment.  Record with the volunteer’s eyes open for 10 seconds.  Do not stop recording.
...
In this exercise, you will compare wave frequencies during different modes of note taking.

1. On the desk in front of the subject, there will be a paper with prompts for your learning activities.

2. Start the recording and drop a comment that says "begin handwritten copying". Have the subject read the first line on the prompt sheet and copy it using the pen and paper in front of them. It is important to keep eyes and head very still while writing to prevent artifacts. Blinking is OK. Drop another comment when they have finished writing.

3. While still recording, drop another comment saying "begin handwritten paraphrasing". Have the subject read the second prompt and paraphrase the sentence using the pen and paper. Drop a comment when finished.

4. For the final handwriting exercise, drop a comment saying "begin handwritten dictation". This time the experimentor will read the third prompt on the sheet out loud to the subject who will copy what they are saying verbatim using pen and paper. Drop a comment when finished.

5. Repeat steps 2-3 two more times. Instead of handwriting print, the subject will run through each trial (copy, paraphrase, and noting dictation) using cursive handwriting and typing on a keyboard. Move through the prompts in order, ensuring each is new. Comment appropriately for each exercise (begin cursive copying, begin typing copying, etc.).

7. For the 10th prompt on the sheet, have the subject read the sentence and draw a diagram/picture to illustrate what the sentence describes.

6. For the 11th prompt on the sheet, have the subject only read the sentence (in their head) without doing any note-taking activity.
...
3. Repeat this exporting process for each combination of note taking mode (handwriting, cursive, typing), experiment (copying, paraphrasing, dictation) as well as drawing and reading only. Also do this process for "eyes open" and "eyes closed" data for control. You should have a total of 13 text files.

4. Make a copy of this Google sheet <insert link> and share with your group members. Select the columns in each text file, copy, and paste them into Sheet 1 under the appropriate heading. If inserted correctly, formulas in Sheet 2 will populate with summed values for waves in the alpha frequency range (8-12 Hz) and beta frequency range (13-30Hz).
...
1. How does the human anatomy make it difficult ot record and electroencephalogram or EEG?
